...
class TestDynamicTablePage:
    @allure.title(
        "Значение в соответствующей ячейки таблицы совпадает с "
    ...
    def test_cpu_from_expected_result_match_value_in_table(self, driver):
        ...
        with allure.step(
            "Проверям, что полученый ожидаемый результат совпадает с значеним в таблице",
        ):
            assert (
                expected_result_cpu
                == dynamic_table_page.get_value_from_table_by_expected_result(
                    expected_result,
                )
            )
