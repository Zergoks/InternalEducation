...
class TestClientSideDelayPage:
    @allure.title(
        "После успешного выполнения логики на стороне пользователя появляется нотификация",
    )
    def test_client_logic_success_notification_is_visible(self, driver):
        ...
        with allure.step(
            "Проверям, что за 30 секунд появляется нотификация успехом запроса",
        ):
            assert (
                client_delay_page.is_element_visible(
                    client_delay_page.SuccessNotification,
                    timeout=30,
                )
                is True
