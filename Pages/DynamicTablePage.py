...
class DynamicTablePage(BasePage):
    ...
    RowWithDataInDynamicTable = (
        '//div[@role="rowgroup"][2]/div[@role="row"]',
        By.XPATH,
    )
    ...
    def __get_index_of_wanted_column(self, expected_result):
        ...
        return (
            header_names.index(wanted_header)
            if wanted_header in header_names
            else False
        )
    ...
    def get_value_from_table_by_expected_result(self, expected_result):
        ...
        wanted_element = (
            f"{self.ContentOfDynamicTable[0]}/div[{wanted_row}]/span[{wanted_column}]",
            By.XPATH,
        )
